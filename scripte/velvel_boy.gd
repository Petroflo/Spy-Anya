...
func _ready():
	...
	connect("body_exited", self, '_on_velvel_boy_body_exited')
...
func _input(event):
	if get_node_or_null('DialogeNode') == null:
		if event.is_action_pressed("ui_accept") and active:
			...
			var dialog = Dialogic.start('timeline-velvel_boy')
...
func _on_velvel_boy_body_entered(body):
	if body.name == 'player':
		active = true

func _on_velvel_boy_body_exited(body):
	if body.name == 'player':
		active = false
